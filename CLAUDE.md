...
- `prototypes/node-link-diagram/index.html` — standalone, self-contained (no build step, no dependencies, `api_answer_model.json` embedded inline) canvas visualization prototype exploring how to present Avi API responses. Renders the JSON as an interactive node-link graph: object/array/VS balls expand on click via a radial layout that eases into place; leaf/scalar values are never drawn as balls, they show in a side panel when their parent ball is clicked. A toggleable cross-reference overlay links balls whose fields share a UUID — resolved refs (the UUID matches another listed object's own `uuid` field, e.g. `vh_parent_vs_ref`) draw straight to that ball, unresolved refs (`pool_ref`, `se_group_ref`, etc., pointing at objects not present in this JSON) get a dedicated synthetic "external object" ball instead. Not wired into the Go server or `web/` build — open `index.html` directly, or serve it with any static file server (`python3 -m http.server`) since some tooling blocks `file://` URLs.
